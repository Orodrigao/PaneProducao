...
test('crédito manual mostra produtos, abatimento e total líquido antes da liberação', async ({ page }) => {
  ...
  await expect(page.getByText('Informe a justificativa do crédito.', { exact: true })).toBeVisible()
  await expect(page.getByRole('checkbox')).toBeDisabled()
  await expect(page.getByRole('button', { name: 'Confirmar cobrança e liberar entrega/coleta' })).toBeDisabled()
  expect(mutations).toHaveLength(0)
  await page.getByLabel('Justificativa', { exact: true }).fill('Crédito combinado no pedido anterior')
  await expect(page.getByText('Informe a justificativa do crédito.', { exact: true })).not.toBeVisible()
