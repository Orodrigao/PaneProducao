...
test('cria sem duplicar após resposta perdida, altera, relê e cancela o mesmo pedido', async ({ page, request }) => {
  ...
  try {
    ...
    await page.route('**/rest/v1/rpc/replace_pj_order_atomic_v2', async route => {
      if (conflictInjected) {
        await route.continue()
        return
      }
      ...
      const body = route.request().postDataJSON() as {
        ...
        p_rows: Array<Record<string, unknown> & { quantity: number }>
        p_expected_rows: Array<{ id: string; updated_at: string }>
      }
      const headers = route.request().headers()
      const externalChange = await request.post(route.request().url(), {
        ...
        data: {
          ...
          p_rows: body.p_rows.map(row => ({ ...row, quantity: 36 })),
        },
      })
      expect(externalChange.ok()).toBe(true)
      await externalChange.dispose()
      await route.continue()
    })
    const replaceResponse = page.waitForResponse(response => response.url().includes('/rpc/replace_pj_order_atomic_v2'))
    await page.getByRole('button', { name: 'Salvar alterações', exact: true }).click()
    const confirmedReplace = await replaceResponse
    expect(confirmedReplace.status()).toBe(400)
    expect(confirmedReplace.request().postDataJSON()).toMatchObject({
    ...
    await expect(page.getByPlaceholder('Digite ou clique pra ver produtos da tabela')).toHaveValue('')

    await page.goto(`/pedidos-pj?legado=1&pedido=${groupId}`)
    await expect(page.getByRole('dialog', { name: 'Ficha do pedido PJ' })).toContainText('Pedido: 36 un', {
      timeout: slowPreviewDataTimeoutMs,
    })

    await page.getByRole('button', { name: 'Editar', exact: true }).click()
    await page.getByLabel('Quantidade de [TESTE] Brioche PJ').fill('2')
    const retryResponse = page.waitForResponse(response => response.url().includes('/rpc/replace_pj_order_atomic_v2'))
    await page.getByRole('button', { name: 'Salvar alterações', exact: true }).click()
