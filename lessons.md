...
2026-08-24 - git-branch-e-integracao - Renomear branch com PR aberta fecha a PR; antes de integrar, `git status -sb` no worktree procurando ahead e commit alheio; antes de apagar, status limpo e PR merged (squash nao e ancestral da main).
...
2026-09-17 - check-copia-worktree-cru - A caixa isolada da Portaria copia o worktree como esta (CRLF do Windows inclusive); teste sensivel a quebra de linha falha so ali, nao no CI real (checkout limpo); normalize o arquivo antes do Check e nunca reaproveite branch de PR ja mesclada.
