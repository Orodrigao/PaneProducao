...
describe('classificação durante a importação da NF-e', () => {
  it('mantém o cadastro acessível sem depender do resultado da busca', () => {
    ...
    // Um botao de cadastro visivel por vez. O permanente cede lugar assim que
    // ha busca: dois botoes com o mesmo nome na mesma tela confundem quem le e
    // quebram o teste de navegador, que clica pelo nome e nao sabe qual e qual
    // (strict mode violation, reprovado em 03/09/2026).
    const blocoPermanente = selectorSource.slice(
      selectorSource.indexOf('2. Produto novo da padaria'),
      conditionalResults,
    )
    expect(blocoPermanente).toContain('!query.trim()')
    expect(blocoPermanente).toContain('onClick={onCreate}')
