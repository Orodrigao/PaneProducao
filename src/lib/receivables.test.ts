...
describe('pedidos PJ a faturar', () => {
  const liberado = { order_group_id: 'p1', dispatched_at: '2026-09-03T10:00:00Z' }
  ...
  it('item sem preco abre o pedido, e nao a tabela de preco', () => {
    // O bloqueio le `unit_price` da linha do PEDIDO, congelada no lancamento.
    // Cadastrar preco na tabela do cliente nao muda pedido ja lancado, entao
    // mandar a Elis para la devolveria ela ao mesmo bloqueio. Corrigir a
    // quantidade tambem nao cria preco nenhum.
    expect(pjOrderBillingBlockLink('item-sem-preco', liberado)).toEqual({
      href: '/pedidos-pj?pedido=p1', label: 'Ver o pedido',
    })
  })
  ...
  it('todo motivo aponta para onde ele se resolve', () => {
    ...
    for (const motivo of Object.keys(PJ_ORDER_BILLING_BLOCK_MESSAGES) as PjOrderBillingBlock[]) {
      const atalho = pjOrderBillingBlockLink(motivo, { order_group_id: 'pedido-1', dispatched_at: '2026-09-03T10:00:00Z' })
      expect(atalho, `motivo sem atalho: ${motivo}`).not.toBeNull()
      expect(atalho?.label.length).toBeGreaterThan(3)
    }
  })

  it('pedido ainda nao liberado nao ganha atalho de corrigir', () => {
    // A Expedicao pode ter conferido tudo e nao ter liberado: ai o motivo vira
    // "nada vai" ou "fora do esperado" com o pedido ainda em aberto, e o
    // formulario de correcao nem existe na janela.
    const naoLiberado = { order_group_id: 'p1', dispatched_at: null }
    expect(pjOrderBillingBlockLink('fora-da-trava', naoLiberado)).toEqual({
      href: '/pedidos-pj?pedido=p1', label: 'Ver o pedido',
    })
    expect(pjOrderBillingBlockLink('nada-enviado', naoLiberado)?.label).toBe('Ver o pedido')
  })

  it('o que a correcao resolve abre o formulario; o resto so abre a tela certa', () => {
    expect(pjOrderBillingBlockLink('fora-da-trava', liberado)?.href).toBe('/pedidos-pj?corrigir=p1')
    expect(pjOrderBillingBlockLink('nada-enviado', liberado)?.href).toBe('/pedidos-pj?corrigir=p1')
    expect(pjOrderBillingBlockLink('sem-conferencia-depois-do-envio', liberado)?.href)
    ...
    expect(pjOrderBillingBlockLink('aguardando-conferencia', liberado)?.href).toBe('/pedidos-pj?pedido=p1')
    expect(pjOrderBillingBlockLink('sem-prazo', liberado)?.href).toBe('/clientes')
  })
  ...
  it('o identificador do pedido vai escapado no endereco', () => {
    ...
    // parametro.
    expect(pjOrderBillingBlockLink('fora-da-trava', { order_group_id: 'a&b=c', dispatched_at: '2026-09-03T10:00:00Z' })?.href)
