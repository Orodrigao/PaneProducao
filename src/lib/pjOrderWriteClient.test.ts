...
describe('tentativas de gravação de Pedidos PJ', () => {
  ...
  it('repete alteração idêntica no mesmo pedido, mas não em outro pedido', () => {
    const firstPayload = {
      rows: [{ quantity: 2 }],
      expectedRows: [{ id: 'row-1', updated_at: '2026-09-10T15:00:00Z' }],
    }
    const first = resolvePjWriteAttempt(null, 'replace', 'group-1', firstPayload, ids('request-1'))
    expect(resolvePjWriteAttempt(first, 'replace', 'group-1', firstPayload, ids('unused'))).toBe(first)

    const newerSnapshot = {
      ...firstPayload,
      expectedRows: [{ id: 'row-1', updated_at: '2026-09-10T15:01:00Z' }],
    }
    expect(resolvePjWriteAttempt(first, 'replace', 'group-1', newerSnapshot, ids('request-2')))
      .toMatchObject({ requestId: 'request-2' })

    const another = resolvePjWriteAttempt(first, 'replace', 'group-2', firstPayload, ids('request-3'))
    expect(another).toMatchObject({ requestId: 'request-3', orderGroupId: 'group-2' })
  })
  ...
  it('envia criação, alteração e cancelamento somente aos contratos atômicos', async () => {
    ...
    const expectedRows = [{ id: 'row-1', updated_at: '2026-09-10T15:00:00Z' }] satisfies PjOrderExpectedRow[]
    const create = resolvePjWriteAttempt(null, 'create', null, rows, ids('request-1', 'group-1'))
    const replace = resolvePjWriteAttempt(null, 'replace', 'group-1', { rows, expectedRows }, ids('request-2'))
    ...
    await replacePjOrder(replace, rows, expectedRows)
  ...
  it('trata somente o conflito conhecido de versão como recusa confirmada', async () => {
    const rows = [{ bread_id: 'bread-1' }] as PjOrderWriteRow[]
    const expectedRows = [{ id: 'row-1', updated_at: '2026-09-10T15:00:00Z' }]
    ...
    rpc.mockResolvedValueOnce({
      data: null,
      error: { code: '40001', message: 'Falha de serialização sem confirmação.' },
    })
    const unknownConflict = await replacePjOrder(attempt, rows, expectedRows).catch(error => error)
    expect(unknownConflict).toMatchObject({ code: '40001', ambiguous: true })
  })
})
