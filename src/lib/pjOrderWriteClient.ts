...
export interface PjOrderExpectedRow {
  id: string
  updated_at: string
}
...
function rpcError(error: { message: string; code?: string }, knownConflict = false): PjOrderWriteError {
  ...
  return new PjOrderWriteError(
    error.message,
    !knownConflict && (!code || !deterministicRefusals.has(code)),
    code,
  )
}
...
export async function replacePjOrder(
  attempt: PjWriteAttempt,
  rows: readonly PjOrderWriteRow[],
  expectedRows: readonly PjOrderExpectedRow[],
): Promise<PjOrderWriteResult> {
  const { data, error } = await supabase.rpc('replace_pj_order_atomic_v2', {
    p_request_id: attempt.requestId,
    p_order_group_id: attempt.orderGroupId,
    p_rows: rows,
