...
export default function ProdutosPage() {
  ...
  const duplicateTriggerRef = useRef<HTMLButtonElement | null>(null)
  ...
  const canDuplicateProducts = !!user
    ...
    && ['/produtos', '*'].some(route => user.allowedRoutes.includes(route))
    && ['/tabelas-preco', '*'].some(route => user.allowedRoutes.includes(route))

  function closeDuplicateProduct() {
    setDuplicateItem(null)
    window.requestAnimationFrame(() => duplicateTriggerRef.current?.focus())
  }

  function openDuplicateProduct(product: Product, trigger: HTMLButtonElement) {
    duplicateTriggerRef.current = trigger
  ...
  return (
    <div className="ps-canvas">
                ...
                <div key={cat} className="ps-card" style={{padding:'4px 14px'}}>
                  ...
                  {items.map(p=>(
                    <div key={p.id} style={{display:'flex', alignItems:'center', gap:8, padding:'10px 0', borderBottom:'1px solid var(--line-soft)', opacity:p.active?1:0.5}}>
                      ...
                      {canDuplicateProducts && <button
                        onClick={event=>openDuplicateProduct(p, event.currentTarget)}
      ...
      {duplicateItem && (
        <div className="ps-sheet-overlay" onClick={e=>e.target===e.currentTarget&&!duplicating&&closeDuplicateProduct()}>
          <div className="ps-sheet" role="dialog" aria-modal="true" aria-labelledby="duplicate-product-title" onKeyDown={event=>{
            if (event.key === 'Escape' && !duplicating) { event.preventDefault(); closeDuplicateProduct() }
            ...
            <div className="ps-fieldgroup" style={{marginBottom:16}}>
              <label className="ps-fieldlabel" htmlFor="duplicate-product-name">Novo nome</label>
              <input
            ...
            <div style={{display:'flex', justifyContent:'flex-end', gap:8}}>
              <button className="ps-btn" onClick={closeDuplicateProduct} disabled={duplicating}>Cancelar</button>
