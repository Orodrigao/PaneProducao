...
### O atalho da correção, e o que o teste do Rodrigo ensinou

O aviso do Contas a Receber dizia o que fazer e não dizia onde. Agora cada
pedido bloqueado tem um atalho que abre o lugar onde ele se resolve, e o de
correção abre o pedido **com o formulário aberto**; ao salvar, a tela devolve a
pessoa ao Contas a Receber. A regra do atalho é `resolvePjOrderShortcut`, função
pura em `src/lib/pjOrderList.ts`, com teste próprio: as duas primeiras versões
do atalho quebraram dentro do componente, uma perdendo o alvo porque limpar o
endereço remontava a página, e outra decidindo a aba por conta própria. **A aba
não é decidida pelo atalho:** quem sabe onde um pedido mora é `organizePjOrders`,
e a regra dela é maior que "foi liberado?" (pedido não liberado com entrega
vencida mora em Fechados para o financeiro). O atalho também só oferece o
formulário de correção em pedido já liberado, porque antes disso o formulário
não existe.

Duas lições ficaram do teste em 07/09:

- **conta fictícia precisa espelhar a real.** O crachá de financeiro do banco de
  teste não tinha `/clientes`, que a Elis tem em produção, então o atalho do
  prazo caía na tela inicial só no preview. Corrigido no `seed.sql`;
- **"Histórico" prometia arquivo morto** e é onde mora uma correção que mexe em
  dinheiro. A aba passou a se chamar **Fechados**.

E uma terceira, da revisão do Sol sobre a versão final: **o atalho de "item sem
preço" apontava para a tela de Tabelas de preço, e não resolvia.** O bloqueio lê
`unit_price` da linha do pedido, congelada no lançamento; cadastrar preço na
tabela do cliente não muda pedido já lançado. O atalho passou a abrir o pedido,
e o recado diz isso com todas as letras. **Este caso ainda não tem conserto
completo na tela:** pedido já liberado com item sem preço não pode ser editado,
e entra na mesma fila de decisão do "nada vai neste pedido".
