...
-- Perfis financeiros ativos ja nascem com a permissao, no mesmo padrao que o
-- Contas a receber usou em 13/08 para as seis permissoes dele.
--
-- POR QUE, e nao "quem concede e o Rodrigo": a tela mostra o botao de corrigir
-- para todo perfil comercial, entao uma permissao sem dono faz o botao aparecer
-- e falhar depois do clique. Foi o que o teste do Rodrigo pegou no preview em
-- 04/09. A concessao automatica e a metade da correcao; a outra metade e a tela
-- so mostrar o botao para quem pode.
--
-- Autorizado por Rodrigo em 2026-09-04. Ninguem e nomeado aqui: a regra e o
-- perfil, e tirar de alguem continua sendo um clique em /admin/usuarios.
insert into public.app_user_permissions (user_id, permission_key, scope, granted_by)
select profile.user_id, 'pedidos_pj.corrigir_quantidade', '*', null::uuid
from public.app_profiles profile
...
as $$
  select case
    ...
    -- O prazo vem antes da conferencia de proposito, e nao por acaso: e a
    -- ordem que o motor ja usava desde 26/08, com a justificativa de que
    -- cliente sem prazo e bloqueio de cadastro que a Elis resolve sozinha, e
    -- que trava TODOS os pedidos daquele cliente. Inverter aqui faria a
    -- mensagem discordar da razao real da recusa, que e o defeito que esta
    -- funcao existe para acabar.
