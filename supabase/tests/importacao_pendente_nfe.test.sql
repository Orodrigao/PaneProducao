...
-- Desde a fase 3A a confirmação recebe também o bloco de totais e a hora de emissão.
select ok(has_function_privilege('authenticated', 'public.confirm_xml_import_draft(uuid, timestamptz, uuid, text, uuid, text, text, date, text, numeric, text, jsonb, jsonb, jsonb, timestamptz)', 'execute'),
  'authenticated confirma rascunho retomado mediante validação interna');
select ok(not has_function_privilege('anon', 'public.confirm_xml_import_draft(uuid, timestamptz, uuid, text, uuid, text, text, date, text, numeric, text, jsonb, jsonb, jsonb, timestamptz)', 'execute'),
