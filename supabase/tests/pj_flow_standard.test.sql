...
values
 ('96000000-0000-4000-8000-000000000001','Contrato PJ Admin','admin','jc',true,'["/pedidos-pj"]'),
 ('96000000-0000-4000-8000-000000000002','Contrato PJ sem acesso','admin','jc',true,'["/pedidos-pj"]'),
 ('96000000-0000-4000-8000-000000000003','Contrato PJ Expedicao','expedicao','jc',true,'["/pedidos-pj"]');
insert into public.app_user_permissions(user_id,permission_key,scope)
select '96000000-0000-4000-8000-000000000001', key, 'jc'
from unnest(array['pedidos_pj.acessar','contas_receber.acessar']) key;
insert into public.app_user_permissions(user_id,permission_key,scope)
select '96000000-0000-4000-8000-000000000003', key, 'jc'
...
values ('96000000-0000-4000-8000-000000000010','[TESTE] Cliente contrato PJ','00000000000096',7,true);
...
values
 ('96000000-0000-4000-8000-000000000010','teste-contrato-pj-a','bread',
  '[TESTE] Pao contrato A',10,'un',1,true),
 ('96000000-0000-4000-8000-000000000010','teste-contrato-pj-b','bread',
...
language sql stable as $$
  select jsonb_build_array(
    jsonb_build_object('bread_id','teste-contrato-pj-a','product_source','bread',
      ...
      'pack_size',1,'pricing_unit','un','customer_id','96000000-0000-4000-8000-000000000010',
...
select lives_ok($q$select public.create_pj_order_atomic(
  '96000000-0000-4000-8000-000000000101','96000000-0000-4000-8000-000000000201',pg_temp.pedido())$q$,
...
select lives_ok($q$select public.create_pj_order_atomic(
  '96000000-0000-4000-8000-000000000102','96000000-0000-4000-8000-000000000202',pg_temp.pedido())$q$,
...
select throws_ok($q$select public.create_pj_order_atomic(
  '96000000-0000-4000-8000-000000000102','96000000-0000-4000-8000-000000000202',pg_temp.pedido(6))$q$,
  '22023',null,'mesmo identificador com conteudo diferente e recusado');
select throws_ok($q$select public.create_pj_order_atomic(
  '96000000-0000-4000-8000-000000000114','96000000-0000-4000-8000-000000000214',
  jsonb_set(pg_temp.pedido(),'{0,unit_price}','0.01'))$q$,
  '22023',null,'preco adulterado fora do catalogo e recusado pelo banco');
reset role;
select is((select count(*)::int from public.orders where order_group_id='96000000-0000-4000-8000-000000000202'),
  2,'repeticao manteve exatamente as duas linhas');
select is((select count(*)::int from private.pj_flow where order_group_id='96000000-0000-4000-8000-000000000202'),
...
select set_config('request.jwt.claim.sub','96000000-0000-4000-8000-000000000002',true);
select throws_ok($q$select public.create_pj_order_atomic(
  '96000000-0000-4000-8000-000000000109','96000000-0000-4000-8000-000000000209',pg_temp.pedido())$q$,
...
values('pj','pj','96000000-0000-4000-8000-000000000299','teste-contrato-pj-a','bread',
  '[TESTE] Pao contrato A',1,10,1,'un','96000000-0000-4000-8000-000000000010',
...
select lives_ok($q$select public.create_pj_order_atomic(
  '96000000-0000-4000-8000-000000000103','96000000-0000-4000-8000-000000000203',pg_temp.pedido())$q$,
  'depois da virada o contrato cria pela jornada padrao');
select lives_ok($q$select public.create_pj_order_atomic(
  '96000000-0000-4000-8000-000000000103','96000000-0000-4000-8000-000000000203',pg_temp.pedido())$q$,
  'repeticao depois da virada tambem e idempotente');
reset role;
select is((select count(*)::int from public.orders where order_group_id='96000000-0000-4000-8000-000000000203'),
  2,'pedido padrao possui todas as linhas uma unica vez');
select is((select activation_mode from private.pj_flow where order_group_id='96000000-0000-4000-8000-000000000203'),
...
begin
  if new.order_group_id='96000000-0000-4000-8000-000000000204' then
...
select set_config('request.jwt.claim.sub','96000000-0000-4000-8000-000000000001',true);
select throws_ok($q$select public.create_pj_order_atomic(
  '96000000-0000-4000-8000-000000000104','96000000-0000-4000-8000-000000000204',pg_temp.pedido())$q$,
...
select is((select count(*)::int from public.orders where order_group_id='96000000-0000-4000-8000-000000000204'),
  0,'falha de inscricao nao deixa linhas soltas');
select is((select count(*)::int from private.pj_order_write_requests where request_id='96000000-0000-4000-8000-000000000104'),
  0,'falha de inscricao nao registra sucesso falso');

set local role authenticated;
select set_config('request.jwt.claim.sub','96000000-0000-4000-8000-000000000001',true);
select lives_ok($q$select public.replace_pj_order_atomic(
  '96000000-0000-4000-8000-000000000105','96000000-0000-4000-8000-000000000202',pg_temp.pedido(8))$q$,
  'edicao atomica do legado continua permitida depois do corte');
select lives_ok($q$select public.replace_pj_order_atomic(
  '96000000-0000-4000-8000-000000000106','96000000-0000-4000-8000-000000000203',
...
select throws_ok($q$update public.orders set order_type='encomenda',store='jc'
  where order_group_id='96000000-0000-4000-8000-000000000202'$q$,
...
select lives_ok($q$select public.schedule_pj_production(private.data_na_padaria(),
  (select jsonb_build_array(jsonb_build_object('order_id',id,'quantity',1,'frozen_quantity',0))
   from public.orders where order_group_id='96000000-0000-4000-8000-000000000203' limit 1),
  '96000000-0000-4000-8000-000000000115')$q$,
...
select lives_ok($q$select public.save_pj_order_dispatch_quantities(
  '96000000-0000-4000-8000-000000000111','96000000-0000-4000-8000-000000000202',
  (select jsonb_agg(jsonb_build_object('order_id',id,'quantity',quantity,'reason',null))
   from public.orders where order_group_id='96000000-0000-4000-8000-000000000202'),null)$q$,
  'contrato legado de conferencia continua funcionando depois do corte');
select lives_ok($q$select public.confirm_pj_order_dispatch('96000000-0000-4000-8000-000000000202')$q$,
  'contrato legado de saida continua funcionando depois do corte');
select lives_ok($q$select public.transition_pj_flow_pilot(
  '96000000-0000-4000-8000-000000000112','96000000-0000-4000-8000-000000000203',0,'save',
  (select jsonb_agg(jsonb_build_object('id',id,'quantity',quantity,'reason',null))
   from public.orders where order_group_id='96000000-0000-4000-8000-000000000203'))$q$,
  'jornada padrao continua salvando a conferencia depois do corte');
select lives_ok($q$select public.transition_pj_flow_pilot(
  '96000000-0000-4000-8000-000000000113','96000000-0000-4000-8000-000000000203',1,'check')$q$,
  'jornada padrao continua concluindo a conferencia');

select set_config('request.jwt.claim.sub','96000000-0000-4000-8000-000000000001',true);
select lives_ok($q$select public.create_pj_order_atomic(
  '96000000-0000-4000-8000-000000000110','96000000-0000-4000-8000-000000000205',
...
select lives_ok($q$select public.cancel_pj_order_atomic(
  '96000000-0000-4000-8000-000000000107','96000000-0000-4000-8000-000000000205','Cliente desistiu')$q$,
  'pedido padrao pode ser cancelado antes da conferencia');
select lives_ok($q$select public.cancel_pj_order_atomic(
  '96000000-0000-4000-8000-000000000107','96000000-0000-4000-8000-000000000205','Cliente desistiu')$q$,
  'repetir cancelamento devolve o mesmo sucesso');
reset role;
select is((select count(*)::int from public.orders where order_group_id='96000000-0000-4000-8000-000000000205'
  and cancelled_at is not null and cancel_reason='Cliente desistiu'),1,
  'cancelamento protege todas as linhas do pedido');
select is((select count(*)::int from private.pj_flow where order_group_id='96000000-0000-4000-8000-000000000205'),
  0,'cancelamento retira o pedido da fila da nova jornada');
select ok(private.is_pj_flow('96000000-0000-4000-8000-000000000205'),
...
select set_config('request.jwt.claim.sub','96000000-0000-4000-8000-000000000001',true);
select ok(public.read_pj_flow_pilot()::text not like '%96000000-0000-4000-8000-000000000205%',
...
select set_config('request.jwt.claim.sub','96000000-0000-4000-8000-000000000001',true);
select throws_ok($q$select public.create_pj_order_atomic(
  '96000000-0000-4000-8000-000000000108','96000000-0000-4000-8000-000000000208',pg_temp.pedido())$q$,
