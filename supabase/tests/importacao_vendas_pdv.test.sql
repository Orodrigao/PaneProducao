...
values
  ('98000000-0000-4000-8000-0000000000f1','sales-imports','cnm/jc/2026-09-12/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.xls','98000000-0000-4000-8000-00000000000a'),
  ('98000000-0000-4000-8000-0000000000f2','sales-imports','cnm/jc/2026-09-12/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb.xls','98000000-0000-4000-8000-00000000000a'),
  ('98000000-0000-4000-8000-0000000000f3','sales-imports','cnm/jc/2026-09-11/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.xls','98000000-0000-4000-8000-00000000000a');
...
select lives_ok($$select public.confirm_sales_import(
  'cnm','jc','sales_by_product','2026-09-12','CNM_JC_2026-09-12.xls',repeat('a',64),
  'cnm/jc/2026-09-12/'||repeat('a',64)||'.xls','cnm-sales-v1',38.90,null,
...
select is((select count(*)::int from public.sales_imports),1,'financeiro vê uma importação');
select is((select count(*)::int from public.sales_import_items),2,'o arquivo entra inteiro em uma confirmação');
select is((select total_net from public.sales_imports where status='confirmed'),38.90::numeric,'total líquido confirmado é preservado');
select is((select public.confirm_sales_import(
  ...
  (select id::text from public.sales_imports where status='confirmed'),'mesmo hash é idempotente');
select throws_ok($$select public.confirm_sales_import(
  'cnm','jc','sales_by_product','2026-09-11','CNM_2026-09-11_JC.xls',repeat('a',64),
...
select throws_ok($$select public.confirm_sales_import(
  'cnm','jc','sales_by_product','2026-09-12','CNM_JC_2026-09-12.xls',repeat('b',64),
  'cnm/jc/2026-09-12/'||repeat('b',64)||'.xls','cnm-sales-v1',10,null,
...
select throws_ok($$select public.confirm_sales_import(
  'cnm','jc','sales_by_product','2026-09-12','CNM_JC_2026-09-12.xls',repeat('b',64),
  'cnm/jc/2026-09-12/'||repeat('b',64)||'.xls','cnm-sales-v1',11,'Correção do fechamento',
  '[{"line_number":5,"external_product_key":"Outro","raw_product_name":"Outro","raw_category":"Pães","quantity":1,"net_total":10,"raw_row":[]}]'::jsonb)$$,
  '22023','A soma dos produtos não fecha com o total informado no arquivo.','diferença financeira bloqueia o lote inteiro');
select lives_ok($$select public.confirm_sales_import(
  'cnm','jc','sales_by_product','2026-09-12','CNM_JC_2026-09-12.xls',repeat('b',64),
  'cnm/jc/2026-09-12/'||repeat('b',64)||'.xls','cnm-sales-v1',10,'Correção do fechamento',
...
select is((select count(*)::int from public.sales_imports),2,'substituição preserva as duas versões');
select is((select count(*)::int from public.sales_imports where status='confirmed'),1,'só uma versão fica ativa');
select is((select count(*)::int from public.sales_imports where status='replaced'),1,'versão anterior fica no histórico');
select is((select count(*)::int from public.sales_import_events where event_type='replaced'),1,'substituição deixa evento de auditoria');
select lives_ok(format($$select public.restore_sales_import(%L::uuid,'Relatório anterior era o correto')$$,
  (select id from public.sales_imports where status='replaced')),'versão anterior pode ser restaurada');
select is((select file_hash from public.sales_imports where status='confirmed'),repeat('a',64),'restauração reativa exatamente o arquivo antigo');
select is((select count(*)::int from public.sales_import_events where event_type='restored'),1,'restauração registra ator e motivo');
...
select throws_ok($$select public.record_sales_day_status('cnm','jc','2026-09-12','closed','Loja fechada')$$,'23505','Este dia já possui venda importada.','dia importado não vira fechado');
select lives_ok($$select public.record_sales_day_status('cnm','jc','2026-09-13','closed','Domingo')$$,'financeiro marca dia fechado com motivo');
select is((select status from public.sales_day_statuses where sale_date='2026-09-13'),'closed','dia fechado fica distinguível de arquivo faltante');
...
select is((select count(*)::int from public.sales_imports),2,'Rodrigo administrador vê o histórico sem concessão granular');
