...
insert into public.products (
  id, name, category, active, unit, kind, is_fabricacao_propria, is_pj,
  production_area, production_process, allows_planned_production,
  allows_unplanned_production, legacy_bread_id
) values
  ('97000000-0000-4000-8000-0000000000d1', '[TESTE] Italiano novo', 'Paes', true, 'un', 'final', true, true,
   'padaria', 'forno', true, true, 'teste-italiano-programacao'),
  ('97000000-0000-4000-8000-0000000000d2', '[TESTE] Sem vinculo', 'Paes', true, 'un', 'final', true, true,
   'padaria', 'forno', true, true, null),
  ('97000000-0000-4000-8000-0000000000d3', '[TESTE] Sem classificacao', 'Paes', true, 'un', 'final', true, true,
   null, null, null, null, null),
  ('97000000-0000-4000-8000-0000000000d4', '[TESTE] Mil folhas montagem', 'Confeitaria', true, 'un', 'final', true, true,
   'confeitaria', 'montagem', true, true, null);
...
insert into public.orders (
  ...
  ('97000000-0000-4000-8000-000000000107', 'pj', 'pj', '97000000-0000-4000-8000-0000000000a5',
   '97000000-0000-4000-8000-0000000000d3', 'product', '[TESTE] Sem classificacao', 8, 'un',
   '97000000-0000-4000-8000-0000000000c1', '[TESTE] Hamburgueria Producao',
   private.data_na_padaria(), private.data_na_padaria() + 2, private.data_na_padaria() + 2, null),
  ('97000000-0000-4000-8000-000000000108', 'pj', 'pj', '97000000-0000-4000-8000-0000000000a6',
   '97000000-0000-4000-8000-0000000000d4', 'product', '[TESTE] Mil folhas montagem', 12, 'un',
   '97000000-0000-4000-8000-0000000000c1', '[TESTE] Hamburgueria Producao',
   private.data_na_padaria(), private.data_na_padaria() + 2, private.data_na_padaria() + 2, null);
...
select throws_ok($$ select * from public.list_pj_production_for_oven_v2(private.data_na_padaria()) $$,
  '42501', 'Usuario sem permissao para consultar a producao PJ.',
  'Comercial nao consulta a carga do Forno');
select throws_ok($$ select * from public.confirm_oven_product_output(
  private.data_na_padaria(), 'product', '97000000-0000-4000-8000-0000000000d2', 1, 0, null, null) $$,
  '42501', 'Usuario sem permissao para confirmar o forno.',
  'Comercial nao confirma a saida do Forno');

select set_config('request.jwt.claim.sub', '97000000-0000-4000-8000-000000000001', true);
select is((select count(*)::int from public.list_pj_production_queue()
  where customer_id = '97000000-0000-4000-8000-0000000000c1'), 7,
...
select ok((select mapping_error is null from public.list_pj_production_queue()
  where order_id = '97000000-0000-4000-8000-000000000103'),
  'produto de Forno classificado nao depende de vinculo legado');
select is((select mapping_error from public.list_pj_production_queue()
  where order_id = '97000000-0000-4000-8000-000000000107'),
  'Produto sem classificacao operacional para producao.',
  'produto sem classificacao continua visivel e bloqueado');
select is((select mapping_error from public.list_pj_production_queue()
  where order_id = '97000000-0000-4000-8000-000000000108'),
  'Pedidos PJ desta area ainda nao recebem programacao. Registre a producao na area responsavel.',
  'montagem fica visivel, mas nao e enviada ao Forno');
...
select lives_ok($$ select public.schedule_pj_production(
  ...
  '[{"order_id":"97000000-0000-4000-8000-000000000101","quantity":300,"frozen_quantity":15},
    {"order_id":"97000000-0000-4000-8000-000000000102","quantity":100,"frozen_quantity":0},
    {"order_id":"97000000-0000-4000-8000-000000000103","quantity":10,"frozen_quantity":0}]'::jsonb,
  '97000000-0000-4000-8000-00000000f103'::uuid) $$,
  'Geolar programa produtos legados, vinculados e sem vinculo');
...
select ok(not exists(select 1 from public.list_pj_production_queue()
  where order_id = '97000000-0000-4000-8000-000000000103'),
  'produto sem vinculo completo tambem sai da pendencia');
...
select is((select quantity from public.list_pj_production_for_oven_v2(private.data_na_padaria())
  where product_source = 'product' and product_id = '97000000-0000-4000-8000-0000000000d2'), 10::numeric,
  'Forno novo recebe produto classificado mesmo sem vinculo');
select ok(not exists(select 1 from public.list_pj_production_for_oven_v2(private.data_na_padaria())
  where product_id = '97000000-0000-4000-8000-0000000000d4'),
  'produto de montagem nunca entra na carga do Forno');
...
select lives_ok($$ select public.schedule_pj_production(
  private.data_na_padaria(),
  '[{"order_id":"97000000-0000-4000-8000-000000000101","quantity":100,"frozen_quantity":0}]'::jsonb,
  '97000000-0000-4000-8000-00000000f105'::uuid) $$,
  'nova decisao permite programar mais do mesmo item no mesmo dia');
select is((select pending_quantity from public.list_pj_production_queue()
  where order_id = '97000000-0000-4000-8000-000000000101'), 200::numeric,
  'segunda programacao parcial preserva o restante correto');

reset role;
select is((select count(*)::int from public.pj_production_schedules
  where request_id = '97000000-0000-4000-8000-00000000f103'), 3,
...
select set_config('request.jwt.claim.sub', '97000000-0000-4000-8000-000000000001', true);
select is((select quantity from public.list_pj_production_for_oven(private.data_na_padaria())
  where bread_id = 'teste-brioche-programacao'), 385::numeric,
...
select lives_ok($$ select * from public.confirm_oven_product_output(
  private.data_na_padaria(), 'product', '97000000-0000-4000-8000-0000000000d2', 9, 1, 'Queimou', null) $$,
  'Forno confirma produto sem identidade antiga');
select is((select quantity_baked from public.production_actuals
  where record_date = private.data_na_padaria()
    and product_source = 'product' and product_id = '97000000-0000-4000-8000-0000000000d2'), 9::numeric,
  'producao realizada guarda a identidade do produto');
select lives_ok($$ select * from public.confirm_oven_product_output(
  private.data_na_padaria(), 'product', '97000000-0000-4000-8000-0000000000d2', 8, 2, 'Queimou', 'Correcao') $$,
  'correcao da confirmacao substitui o saldo, sem duplicar');
select is((select quantity from public.bread_movements
  where movement_type = 'forno_entrada' and reference_type = 'production_actual'
    and product_source = 'product' and product_id = '97000000-0000-4000-8000-0000000000d2'), 8::numeric,
  'estoque conserva apenas o ultimo total confirmado');
select is((select count(*)::int from public.production_actual_events
  where record_date = private.data_na_padaria()
    and product_source = 'product' and product_id = '97000000-0000-4000-8000-0000000000d2'), 2,
  'historico preserva as duas confirmacoes');
select lives_ok($$ select * from public.confirm_oven_product_output(
  private.data_na_padaria(), 'product', '97000000-0000-4000-8000-0000000000d1', 7, 0, null, null) $$,
  'produto com ponte converge para a confirmacao do pao legado');
select ok(not exists(select 1 from public.production_actuals
  where record_date = private.data_na_padaria()
    and product_source = 'product' and product_id = '97000000-0000-4000-8000-0000000000d1'),
  'ponte legada nao cria um segundo realizado para o produto');
select is((select quantity_baked from public.production_actuals
  where record_date = private.data_na_padaria() and product_source = 'bread'
    and product_id = 'teste-italiano-programacao'), 7::numeric,
  'ponte legada grava somente no lote canonico antigo');

reset role;
update public.products set active = false, production_process = 'montagem'
where id = '97000000-0000-4000-8000-0000000000d2';
set local role authenticated;
select set_config('request.jwt.claim.sub', '97000000-0000-4000-8000-000000000001', true);
select lives_ok($$ select * from public.confirm_oven_product_output(
  private.data_na_padaria(), 'product', '97000000-0000-4000-8000-0000000000d2', 7, 0, null, 'Correcao apos reclassificacao') $$,
  'lote ja confirmado pode ser corrigido depois de reclassificacao');
select is((select quantity_baked from public.production_actuals
  where record_date = private.data_na_padaria()
    and product_source = 'product' and product_id = '97000000-0000-4000-8000-0000000000d2'), 7::numeric,
  'correcao conserva a identidade e o snapshot do lote');

reset role;
select throws_ok($$ insert into public.bread_movements (
  movement_type, bread_id, product_source, product_id, location, quantity, recorded_by
) values ('forno_entrada', 'teste-brioche-programacao', 'product',
  '97000000-0000-4000-8000-0000000000d2', 'central', 1, 'Teste') $$,
  '23514', null,
  'movimento recusa identidade divergente entre produto e pao');
select throws_ok($$ insert into public.bread_movements (
  movement_type, bread_id, product_source, product_id, location, quantity, recorded_by
) values ('forno_entrada', null, 'bread', 'teste-brioche-programacao', 'central', 1, 'Teste') $$,
  '23514', null,
  'identidade de pao exige o bread_id correspondente, sem aceitar nulo');
...
select is((select count(*)::int from public.pj_production_schedules), 5,
