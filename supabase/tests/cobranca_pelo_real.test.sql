...
select is(
  (select amount from public.receivables where id = private.build_receivable_from_pj_order(
...
-- E o carimbo da CONFERENCIA, ao contrario, precisa avancar: e ele que diz
-- quando o numero mudou pela ultima vez, e a proxima tela usa isso como versao.
select is((select count(*)::int
    from public.orders atual
    join carimbo_antes anterior on anterior.id = atual.id
    where atual.dispatched_quantity_at is not distinct from anterior.dispatched_quantity_at), 0,
  'e o carimbo da conferencia avanca nas linhas corrigidas');
