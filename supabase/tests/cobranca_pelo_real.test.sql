...
-- A permissao chega sozinha a quem e financeiro ------------------------------
-- O teste do Rodrigo em 04/09 esbarrou nisto: a permissao nascia sem dono e o
-- botao aparecia para o financeiro sem funcionar. A migration passou a conceder
-- a todo perfil financeiro ativo, e este teste prova que continua assim.
select ok(exists(
    select 1
    from public.app_profiles profile
    join public.app_user_permissions concessao on concessao.user_id = profile.user_id
    where profile.role = 'financeiro'
      and profile.active
      and concessao.permission_key = 'pedidos_pj.corrigir_quantidade'
  ),
  'perfil financeiro ativo nasce com a permissao de corrigir');
